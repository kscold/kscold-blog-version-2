...
test('홈의 소개와 탐색 링크는 서버 HTML에 있고 데스크톱 사이드바는 홈에서만 숨긴다', async ({ page, request }) => {
  ...
  expect(html).toContain('home-manifesto');
  expect(html).toContain('home-connections');
  await page.goto('/');
  ...
  const rail = page.getByLabel('블로그 기능 탐색');
  for (const href of ['/blog', '/feed', '/vault', '/?chat=open', '/admin-night']) {
...
test('움직임 줄이기에서는 3D와 고정 연출 없이 모든 탐색 단계를 읽을 수 있다', async ({ page }) => {
  ...
  for (let index = 0; index < 4; index++) await expect(page.locator(`[data-preview="${index}"]`)).toBeVisible();
...
for (const width of [390, 768, 1440]) {
  test(`홈 콘텐츠와 가로 카드가 문서 가로 넘침을 만들지 않는다: ${width}`, async ({ page }) => {
    ...
    await page.goto('/');
    await page.getByRole('heading', { name: /흩어진 생각이/ }).scrollIntoViewIfNeeded();
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    const lastCard = page.getByLabel('블로그 기능 탐색').locator('a').last();
