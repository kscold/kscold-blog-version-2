...
test('태그 인기순은 전체 결과를 정렬하고 페이지 이동과 새로고침에 유지된다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 2, name: /^CI public/ }).first()).toHaveText('CI public 19 글');
  ...
  await expect(page.getByRole('heading', { level: 2, name: /^CI public/ }).first()).toHaveText('CI public 7 글');
  await page.reload();
  ...
  await expect(page.getByRole('heading', { level: 2, name: /^CI public/ }).first()).toHaveText('CI public 1 글');
});
