...
export default function FloatingChatButton({ onClick, unreadCount = 0 }: FloatingChatButtonProps) {
  ...
  return (
    <motion.button
      ...
      whileHover={supportsHover && allowRichEffects ? { scale: 1.1 } : undefined}
    ...
    >
      ...
      <div className="relative flex h-14 w-14 items-center justify-center rounded-full border border-surface-800 bg-surface-900 shadow-xl shadow-surface-900/10 transition-all duration-300 hover:bg-surface-800 sm:h-16 sm:w-16">
        ...
        {unreadCount > 0 && (
          <motion.div
            ...
            initial={allowRichEffects ? { scale: 0 } : false}
            animate={allowRichEffects ? { scale: 1 } : undefined}
            transition={allowRichEffects ? { type: 'spring', stiffness: 500 } : undefined}
          >
      ...
      {/* 툴팁 */}
      <AnimatePresence>
        {supportsHover && allowRichEffects && isHovered && (
          <motion.div
