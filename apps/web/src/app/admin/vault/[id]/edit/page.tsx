...
import { useVaultFolders, useUpdateVaultNote } from '@/hooks/useVault';
import { apiClient } from '@/lib/api-client';
import { MarkdownContent } from '@/components/blog/MarkdownContent';
import { VaultNote, VaultNoteUpdateRequest, VaultFolder } from '@/types/api';
...
export default function EditVaultNotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const noteData: VaultNoteUpdateRequest = {
      title,
      slug: slug || undefined,
      content,
      folderId,
    ...
    try {
      await updateNote.mutateAsync({ id, data: noteData });
      alert('노트가 수정되었습니다.');
      router.push('/admin/vault');
    } catch (error: any) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  >
                    {updateNote.isPending ? '저장 중...' : '노트 수정'}
                  </button>
                  <button
                    type="button"
                    onClick={() => router.back()}
