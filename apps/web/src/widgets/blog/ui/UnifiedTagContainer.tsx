...
export function UnifiedTagContainer({ tagName }: UnifiedTagContainerProps) {
  ...
  const { data: tagIndex = [], isLoading: tagIndexLoading } = useTagIndex();
  const tagUsage = tagIndex.find(
    tag => tag.name.toLowerCase() === tagName.toLowerCase() || tag.slug === tagName
  );
  const canonicalTagName = tagUsage?.name ?? tagName;
  ...
  const postCount = tagUsage?.postCount ?? postsData?.totalElements ?? 0;
  const feedCount = tagUsage?.feedCount ?? feedsData?.totalElements ?? 0;
  const isPostsLoading = tagIndexLoading || postsLoading;
  const isFeedsLoading = tagIndexLoading || feedsLoading;
    ...
    <div className="min-h-screen bg-surface-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          ...
          <span className="text-surface-900 font-medium">#{canonicalTagName}</span>
        </motion.nav>
        ...
        {/* 둘 다 없을 때 */}
        {!isPostsLoading && !isFeedsLoading && postCount === 0 && feedCount === 0 && (
          <div className="text-center py-24">
            <h2 className="text-2xl font-black text-surface-900 mb-2">콘텐츠가 없습니다</h2>
            <p className="text-surface-500 mb-6">
              #{canonicalTagName} 태그에 연결된 포스트나 피드가 없습니다.
            </p>
