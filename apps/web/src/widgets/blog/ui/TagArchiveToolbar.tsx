...
export function TagArchiveToolbar({ basePath, sort, page, initialPosts }: TagArchiveData) {
  return (
    <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
      <p className="text-sm text-surface-500" aria-live="polite">
        {initialPosts.totalElements > 0 ? `${page} / ${initialPosts.totalPages} 페이지 · ` : ''}
        {sort === 'popular' ? '조회수가 높은 순' : '최근 발행한 순'}
      </p>
      <nav aria-label="태그 포스트 정렬" className="flex gap-2">
        {(['latest', 'popular'] as const).map(value => (
          <Link key={value} href={value === 'latest' ? basePath : `${basePath}?sort=popular`}
