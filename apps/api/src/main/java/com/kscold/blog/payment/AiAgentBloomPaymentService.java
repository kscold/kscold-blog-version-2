...
public class AiAgentBloomPaymentService {
    ...
    private static final DateTimeFormatter PAYMENT_ID_DATE = DateTimeFormatter.ofPattern("yyyyMMddHHmmss");
    ...
    public CompletePaymentResponse complete(String userId, String paymentId, String requestPaymentAccessToken) {
        ...
        if (!"PAID".equals(portOneStatus)) {
            String message = "결제가 완료되지 않았습니다. status=" + (portOneStatus.isBlank() ? "UNKNOWN" : portOneStatus);
            order.markFailed(portOneStatus, message);
            paymentOrderRepository.save(order);
            throw InvalidRequestException.invalidInput(message);
        }
        if (paidAmount != TOTAL_AMOUNT) {
            String message = "결제 금액이 일치하지 않습니다. expected=%d, actual=%d".formatted(TOTAL_AMOUNT, paidAmount);
            order.markFailed(portOneStatus, message);
            paymentOrderRepository.save(order);
            throw InvalidRequestException.invalidInput(message);
        }
        ...
        paymentOrderRepository.save(order);
        return new CompletePaymentResponse(order.getPaymentId(), order.getStatus(), order.getPortOneStatus(), "결제가 확인되었습니다.");
    }

    private JsonNode fetchPortOnePayment(String paymentId) {
        try {
            JsonNode payment = restClientBuilder
                    .baseUrl(portOnePaymentProperties.getApiBaseUrl())
                    .defaultHeader(HttpHeaders.AUTHORIZATION, "PortOne " + portOnePaymentProperties.getApiSecret())
                    .build()
                    .get()
            ...
            log.warn("포트원 결제 조회 실패 paymentId={}, status={}", paymentId, exception.getStatusCode(), exception);
