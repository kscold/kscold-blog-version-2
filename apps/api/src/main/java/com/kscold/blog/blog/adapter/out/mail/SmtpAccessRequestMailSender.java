...
public class SmtpAccessRequestMailSender implements AccessRequestMailSender {
    ...
    public void sendApproved(
        ...
        String postUrl =
                StringUtils.hasText(request.getPostSlug())
                        ? base + "/blog/" + request.getPostSlug()
                        : base + "/blog";

        String target =
                request.getGrantScope() == AccessRequest.GrantScope.CATEGORY
    ...
    private String buildHtml(String displayName, String target, String url) {
        return "<div style=\"font-family:system-ui,-apple-system,sans-serif;max-width:520px;margin:0 auto;padding:32px;color:#1a1a2e;\">"
                ...
                + "<a href=\""
                + url
                + "\" style=\"display:inline-block;padding:12px 24px;background:#1a1a2e;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;\">글 보러 가기</a>"
